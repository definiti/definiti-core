...
export function verifyYearPeriod(period: $Period): string|null {
  return verify("The period must last one year", () => {
    return ((((period).end).timestamp).minus(((period).start).timestamp)).equals(((new NumberWrapper(365)).time(new NumberWrapper(24))).time(new NumberWrapper(3600)));
  })
}
...
interface $MultiplePeriod  {
  periods: ListWrapper<Period>;
}
     

export interface MultiplePeriod extends $MultiplePeriod {}

export function MultiplePeriod(periods: ListWrapper<Period>): string|Readonly<MultiplePeriod> {
  const __result = {periods};
  const multiplePeriod = __result;
  let __errorOpt = null;
  const __verifications = [
    
verify("end should be after start", () => {
  return (((multiplePeriod).periods).nonEmpty()) && ((((((multiplePeriod).periods).head).start).timestamp).upper(new NumberWrapper(0)));
})
...
interface $NonEmptyList<A>  {
  values: ListWrapper<A>;
}
     

export interface NonEmptyList<A> extends $NonEmptyList<A> {}

export function NonEmptyList<A>(values: ListWrapper<A>): string|Readonly<NonEmptyList<A>> {
  const __result = {values};
  const nonEmptyList = __result;
  let __errorOpt = null;
  const __verifications = [
    
verify("The list should not be empty", () => {
  return ((nonEmptyList).values).nonEmpty();
})
...
export interface NonEmptyPeriodList extends $NonEmptyList<Period> {}

export function NonEmptyPeriodList(values: ListWrapper<Period>): string|Readonly<NonEmptyPeriodList> {
  const __result = {values};
  const nonEmptyList = __result;
  let __errorOpt = null;
  const __verifications = [
