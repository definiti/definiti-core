...
import definiti.parser.ExpressionParser
    ...
    def process(acc: Syntax, remaining: Syntax): Syntax = remaining match {
      ...
      case (function: FunctionToken) :: tail =>
        val newToken = FunctionExpressionToken(
          ...
          body = CombinedExpressionToken(ExpressionParser.splitExpressionsFromBrace(function.body).map(ExpressionParser.buildExpression)).simplify()
        )
      ...
      case (structuredVerificationToken: StructuredVerificationToken) :: tail =>
        val newToken = VerificationExpressionToken(
          ...
          FunctionExpressionToken(
            ...
            body = CombinedExpressionToken(ExpressionParser.splitExpressionsFromBrace(structuredVerificationToken.function.body).map(ExpressionParser.buildExpression)).simplify()
          )
        )
        process(acc :+ newToken, tail)
          ...
          structuredDefinedTypeToken.definedVerifications.map { definedVerification =>
            TypeVerificationExpressionToken(
              ...
              FunctionExpressionToken(
                ...
                body = CombinedExpressionToken(ExpressionParser.splitExpressionsFromBrace(definedVerification.function.body).map(ExpressionParser.buildExpression)).simplify()
              )
            )
          },
