...
private[core] class ProjectParser(configuration: Configuration) {
  ...
  private def buildDefinitiAST(): Validated[Root] = {
  ...
  private def parseDefinitiFile(source: String): Validated[RootFile] = {
  ...
  private def buildCoreDefinitionAST(): Validated[Seq[ClassDefinition]] = {
  ...
  private def parseCoreDefinitionFile(source: String): Validated[Seq[ClassDefinition]] = {
    val errorListener = new ErrorListener(source)
